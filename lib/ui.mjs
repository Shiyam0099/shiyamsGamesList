const paths={
 ...
 news:'<rect x="3" y="3" width="18" height="18" rx="2"/><path d="M7 7h4v4H7zM15 7h2m-2 4h2M7 15h10M7 18h7"/>',
...
export {brand} from './branding.mjs';
...
export function decorateFields(root=document){
 for(const control of root.querySelectorAll('input:not([type="checkbox"]):not([type="hidden"]),select,textarea')){
  ...
  const kind=name==='calendar-order'?'sort':name==='calendar-platform'?'game':control.type==='password'?'lock':control.type==='email'?'mail':/search/.test(name)?'search':name==='article-source'?'news':/username/.test(name)?'user':/year|checked|calendar/.test(name)?'calendar':/metacritic$/.test(name)?'star':/video/.test(name)?'video':/genre|platform/.test(name)?'tag':/status/.test(name)?'status':/sort|order/.test(name)?'sort':control.type==='url'?'link':control.tagName==='TEXTAREA'?'text':/rec-mode/.test(name)?'shuffle':'game';
