...
(async () => {
  ...
  const total=fixtureGames.length,counts=fixtureGames.reduce((a,g)=>(a[g.status]=(a[g.status]||0)+1,a),{});
  ...
  async function chooseGenre(value){await page.locator('#collection-genre-toggle').click();if(!value){await page.getByRole('button',{name:'Show all genres',exact:true}).click();return;}await page.locator('#collection-genre-search').fill(value);await page.getByRole('option',{name:value,exact:true}).click();}
  ...
  assert.equal(await page.locator('#stats .stat strong').allTextContents().then(a=>a.join(',')), [total,counts.unplayed,counts.loved,counts.dropped].join(','));
  assert.equal(await page.locator('.rate-tried strong').textContent(), `${((counts.loved+counts.dropped)/total*100).toFixed(1)}%`);
  ...
  await page.keyboard.press('Home'); assert.equal(await page.locator('#grid .card').count(), total);
  assert.equal(await page.locator('#tab-all span').textContent(), String(total));
  assert.equal(await page.locator('#result-count').textContent(), `${total} of ${total} games`);
  ...
  assert.equal(await page.locator('#grid .card').count(), counts.unplayed);
