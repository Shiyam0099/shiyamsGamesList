import {genreCatalogue,normalizeGenres,genreMatches} from '../lib/genres.mjs';
...
export function bindGenrePicker(root,initial=[],existing=[]){
 const selected=new Set(normalizeGenres(initial)),choices=normalizeGenres([...genreCatalogue,...existing,...initial]).sort((a,b)=>a.localeCompare(b));
 ...
 function renderOptions(){
  const query=search.value.trim(),matches=choices.filter(g=>genreMatches(g,query) || (query && normalizeGenres([query]).includes(g)));
  ...
  if(query && normalizeGenres([query]).some(g=>!choices.includes(g)))list.insertAdjacentHTML('beforeend',`<button type="button" class="genre-add">${icon('plus')} Add “${escape(query)}”</button>`);
 ...
 list.onclick=event=>{if(isBusy() || !event.target.closest('.genre-add'))return;const value=search.value.trim();
  ...
  if(!value)return;const additions=normalizeGenres([value]);if(new Set([...selected,...additions]).size>30){feedback.textContent='You can select up to 30 genres.';return;}for(const genre of additions){if(!choices.includes(genre))choices.push(genre);selected.add(genre);}choices.sort((a,b)=>a.localeCompare(b));search.value='';renderSelection();renderOptions();search.focus();
 };
